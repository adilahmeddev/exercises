...
func NewAdder(history map[int]int) *Adder {
	return &Adder{history: history}
}
...
func (a Adder) FormatNumber(number int) string {
	commaCount := 0
	...
	if number <= 9999 {
		...
		for i := len(chars); i >= 0; i-- {
			if (len(chars)-i-commaCount)%3 == 0 && i != 0 && i != len(chars) {
				chars = a.InsertIntoSlice(chars, i, ',')
				commaCount++
			}
		}
		return string(chars)
...
func (a Adder) InsertIntoSlice(slice []rune, index int, char rune) []rune {
